...
def importation_page():
    ...
    for tab in tabs:
        if st.session_state.page == tab:
            st.sidebar.write(f'<div style="color:blue; text-align: left;">{tab}</div>', unsafe_allow_html=True)
        else:
            st.sidebar.write(f'<div style="text-align: left;">{tab}</div>', unsafe_allow_html=True)
