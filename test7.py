...
def init_session():
    if "page" not in st.session_state:
        ...
        st.session_state.suivant_button_disabled = True  # Initialiser le bouton "Suivant" comme désactivé
...
def importation_page():
    ...
    if uploaded_file is not None:
        ...
        # Activer le bouton "Suivant" après l'importation du fichier
        st.session_state.suivant_button_disabled = False
    ...
    st.title("Affichage des premières lignes du fichier CSV")
    ...
    if uploaded_file is not None:
        try:
            ...
            st.write("Aperçu des premières lignes du fichier:")
            st.write(df.head())
    ...
    with st.form(key="importation_form", border=False):
        ...
        st.form_submit_button("Suivant", on_click=lambda: st.session_state.update({"page": 2}), disabled=st.session_state.suivant_button_disabled)
